...
import com.example.ezwordmaster.ui.common.QuizActionButton
import com.example.ezwordmaster.ui.common.QuizOption
    ...
    Box(
        ...
        Column(
            ...
            AnimatedContent(
                ...
                val currentQ = state.questions.getOrNull(it)
                QuizQuestionCard(
                    questionText = currentQ?.questionWord?.word ?: "Đang tải...",
                    answerText = currentQ?.displayedMeaning ?: "",
                    helperText = currentQ?.displayedMeaning ?: "Đang tải nghĩa...",
                    showAnswer = true,
            ...
            Column(
                ...
                AnimatedVisibility(
                    ...
                    Row(
                        ...
                        QuizOption(
                            text = "SAI",
                            isSelected = state.selectedTrueFalse == false,
                            showResult = state.showResult,
                            isCorrect = state.lastAnswerCorrect == true && state.selectedTrueFalse == false,
                            showAnswer = showAnswer,
                            enabled = !state.showResult,
                            onClick = {
                                viewModel.onTrueFalseSelect(false)
                                viewModel.submitTrueFalse()
                            },
                            modifier = Modifier
                                .weight(1f)
                                .height(80.dp)
                        ...
                        QuizOption(
                            text = "ĐÚNG",
                            isSelected = state.selectedTrueFalse == true,
                            showResult = state.showResult,
                            isCorrect = state.lastAnswerCorrect == true && state.selectedTrueFalse == true,
                            showAnswer = showAnswer,
                            enabled = !state.showResult,
                            onClick = {
                                viewModel.onTrueFalseSelect(true)
                                viewModel.submitTrueFalse()
                            },
                            modifier = Modifier
                                .weight(1f)
            ...
            QuizActionButton(
                showAnswer = showAnswer,
                userAnswerIsNotBlank = state.selectedTrueFalse != null,
                isCompleted = state.isCompleted,
                showResult = state.showResult,
                onNext = { viewModel.nextQuestion() },
                onSubmit = { viewModel.submitTrueFalse() },
                onComplete = {
